...
DATA = REPO / 'data'                             # PUBLIC: served to the site at /data/
PRIV = ROOT / 'private'                          # PRIVATE: gitignored, never published
...
def load():
    ...
    profile = json.loads((PRIV / 'profile.json').read_text(encoding='utf-8'))
    activity = json.loads((PRIV / 'activity.json').read_text(encoding='utf-8'))
...
def main():
    ...
    # the site page is written once, straight to the Pages root beside index.html
    (REPO / 'publications.html').write_text(render_html(pubs, profile), encoding='utf-8')
    ...
    # PUBLIC profile: only the fields index.html reads. Leadership notes, editorial
    # notes, esteem and verification flags stay in cv/private/profile.json.
    m = profile['metrics']
    (DATA / 'profile.json').write_text(json.dumps({
        ...
        'metrics': {'verified': m['verified'],
                    'all_time': m['all_time'],
                    'since_2021': m['since_2021']},
        'headline_numbers': profile['headline_numbers'],
