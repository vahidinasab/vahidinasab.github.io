...
├── index.html                    the front page (counters still hard-coded: see below)
├── publications.html             GENERATED. Overwritten by build.py on every run.
├── data/                         served to the site, and the source of truth
│   ├── publications.json           173 itemised outputs
│   ├── profile.json                identity, metrics, leadership and editorial roles, esteem
...
│   ├── master-part1.md           Part I of the master record. The only prose edited by hand.
...
│   └── out/                      GENERATED. Never edit anything in here.
├── .github/workflows/validate.yml
└── .nojekyll                     stops Jekyll touching data/ and cv/
```
...
| `cv/out/site-metrics.json` | the front-page counters |
